# Evals & Replay System — Architecture

## Overview

The eval system provides a measurable evaluation framework for Altos agent sessions. It records sessions to disk, runs regression scenarios against real `AgentRuntime` instances, and scores outcomes against expected behavior. This enables reproducible testing of agent behavior without requiring AI API calls in CI.
...
The GitHub Actions workflow at `.github/workflows/evals.yml` runs on:
- Every push to `main`
- Every pull request
- On-demand via `workflow_dispatch`

**CI test matrix:**
1. Core package tests: `pnpm run test --filter="@altos/*"` and typecheck: `pnpm run typecheck --filter="@altos/*"`
2. Core evals (4 deterministic scenarios that do not require AI API keys):
   - `read-only-repo`
   - `simple-file-edit`
   - `permission-denial`
   - `dangerous-command-refusal`

The remaining scenarios (`test-failure-fix`, `plugin-tool-execution`, `memory-search`, `repo-map-generation`) require additional setup or AI API credentials and are run manually or in extended CI pipelines.
...
```
~/.altos/sessions/<session-id>/
├── metadata.json    — SessionMetadata: cwd, model, timestamps, outcome, tokenUsage
├── session.jsonl    — RecordedEvent[]: one JSON event per line
└── eval-result.json — EvalResult (only present if run via altos eval)
```

Sessions can be replayed with `altos replay <session-id>` to re-run against fresh runtime and compare outcomes.
