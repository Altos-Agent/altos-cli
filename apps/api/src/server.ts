...
export const buildServer = async (options: BuildServerOptions = {}) => {
  ...
  await registerEmergencyPauseRoutes(server, dbClient);
  await registerChainRoutes(server, dbClient);
  await registerManagementRoutes(server, dbClient);
  await registerApprovalRoutes(server, dbClient);
  ...
  await registerTelegramRoutes(server, dbClient);
  await createSchedulerRoutes(dbClient)(server);
  await registerPlanRoutes(server, dbClient);
  await registerRiskRoutes(server, dbClient);
  await registerTradeRoutes(server, dbClient);
  await registerTransactionRoutes(server, dbClient);
  await registerWalletRoutes(server, dbClient);
